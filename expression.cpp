...
extern void
  enter(BlockLocal *block_local, const ALFA &identifier, Objects objects);
auto loc(BlockLocal *block, const ALFA &ID) -> int64_t;
auto type_compatible(Item /*X*/, Item /*Y*/) -> bool;
extern void
  BLOCK(Interpreter *il, SymbolSet fsys, bool ISFUN, int LEVEL, int PRT);
...
auto COMPATIBLE(Item V, Item W) -> bool {
  ...
  if (V.types == W.types) {
    switch (V.types) {
      ...
      case Types::CHARS:
      case Types::BOOLS:
      case Types::LOCKS: rtn = true; break;
      case Types::RECS:
      ...
      case Types::PNTS:
        ...
        break;
      case Types::ARRAYS: ARRAY_COMPATIBLE(V, W); break;
      default: break;
    }
  } else
    rtn = false;
...
auto ARRAY_COMPATIBLE(Item V, Item W) -> bool {
  ...
  while (V.types == Types::ARRAYS && W.types == Types::ARRAYS) {
    if (
      ATAB[V.reference].HIGH != ATAB[W.reference].HIGH ||
...
auto type_compatible(Item x, Item y) -> bool {
  ...
  // LOCKS)))
  if (
    (standard_set.test(static_cast<int>(x.types)) || x.types == Types::LOCKS) &&
...
auto FACTOR(BasicLocal *bx, SymbolSet &FSYS, Item &X) -> void {
  ...
  if (symbol == Symbol::IDENT) {
    ...
    switch (TAB[I].object) {
      ...
      case Objects::Component: error(44); break;
        ...
        if (TAB[I].LEV != 0)
          ...
          if (symbol == Symbol::IDENT) {
            ...
            if (symbol == Symbol::IDENT) {
              int I = loc(bx->block_local, ID);
              if (I != 0) {
                if (TAB[I].object != Objects::StructTag)
                  error(108);
                else
                  emit(24, TAB[I].address);
              }
    ...
    if (symbol == Symbol::IDENT) {
      ...
      if (TAB[I].object == Objects::Type1) {
        ...
        if (symbol != Symbol::RPARENT)
          ...
          in_symbol();
        if (
          !standard_set.test(static_cast<int>(TYPCAST)) &&
          TYPCAST != Types::PNTS)
          error(121);
        FACTOR(bx, FSYS, X);
        if (X.is_address)
        ...
        if (TYPCAST == Types::INTS) {
          ...
          if (X.types != Types::PNTS || CTAB[X.reference].ELTYP != Types::VOIDS)
            error(121);
        } else
          error(121);
        X.types = TYPCAST;
    ...
    if (X.is_address)
      LOADVAL(X);
    if (
      X.types == Types::INTS || X.types == Types::REALS ||
...
void TYPF(
  BlockLocal *bl, SymbolSet FSYS, Types &types, int64_t &RF, int64_t &SZ) {
  //    int64_t X, I, J;
  //    TYPES ELTP;
  ...
  if (type_set.test(static_cast<int>(symbol))) {
    if (
      symbol == Symbol::SHORTSY || symbol == Symbol::LONGSY ||
    ...
    if (symbol == Symbol::IDENT) {
      ...
      if (symbol == Symbol::IDENT) {
        ...
        in_symbol();
        if (symbol == Symbol::LSETBRACK) {
          enter(bl, tl.TID, Objects::StructTag);
          tl.TS = tab_index;
          ...
          tl.X = loc(bl, tl.TID);
          if (tl.X != 0) {
            if (TAB[tl.X].object != Objects::StructTag)
              error(108);
            else {
              tl.TP = Types::RECS;
              tl.RF = TAB[tl.X].reference;
...
void PARAMETERLIST(BlockLocal *bl) {
  ...
  while (type_set.test(static_cast<int>(symbol)) || symbol == Symbol::VALUESY) {
    bool VALUEFOUND;
...
L56:
  ...
  if (symbol != Symbol::SEMICOLON) {
    if (symbol == Symbol::LSETBRACK) {
      ...
      if (proto_index >= 0) {
        if (
          bl->PCNT != BTAB[proto_ref].PARCNT || bl->DX != BTAB[proto_ref].PSIZE)
          error(152);
      }
    } else {
      sv = 0;
...
void FUNCDECLARATION(BlockLocal *bl, Types TP, int64_t RF, int64_t SZ) {
  ...
  if (proto_index >= 0) {
    ...
    if (
      !type_compatible(return_type, PRORET) ||
      return_type.types != PRORET.types)
      error(152);
    proto_ref = TAB[T0].reference;
  }

  TAB[T0].object = Objects::Function;
  TAB[T0].types = TP;
  TAB[T0].FREF = RF;
  TAB[T0].LEV = bl->LEVEL;
  TAB[T0].normal = true;
...
void PROCDECLARATION(BlockLocal *bl) {
  ...
  if (proto_index >= 0) {
    T0 = proto_index;
    if (TAB[T0].object != Objects::Procedure)
      error(152);
    proto_ref = TAB[T0].reference;
  }
    ...
    do {
      ...
      if (symbol == Symbol::LPARENT) {
        ...
        if (symbol == Symbol::BECOMES) {
          if (TP == Types::CHANS || TP == Types::LOCKS || TP == Types::RECS)
            error(136);
