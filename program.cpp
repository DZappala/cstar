...
auto PRE_BUF = std::make_unique<PreBuffer>(PreBuffer());

bool interactive = false;

static void enter(const char* X0, Objects X1, Types X2, int X3) {
  tab_index++;
  if (tab_index == TMAX) fatal(1);
  else {
...
auto to_string(const IOError err) -> const char* {
  switch (err) {
    ...
    case IOError::BufferOverflow: return
        ...
        "can be changed to increase the buffer size";
...
auto program() -> std::expected<void, ProgramError> {
  ...
  const auto cmd = std::make_unique<Commander>();
  cmd.interpret();
  std::cout << '\n';
  ...
  if (OUTPUTFILE_FLAG) OUTPUT.close();
  if (INPUTFILE_FLAG) INPUT.close();
  if (SOURCEOPEN_FLAG) SOURCE.close();
  return {};
...
void cs_error(T err, int err_num = 0, string context = "") {
...
auto main(int argc, const char* argv[]) -> int {
  ...
  if (auto res = check_file(from_file_path, from_file); !res)
    cs_error(res.error());

  if (auto res2 = make_prebuf(from_file_path, temp_buf, prebuffer_overhead);
    !res2)
    cs_error(res2.error());

  [[maybe_unused]] auto prog = program().or_else(
