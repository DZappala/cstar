...
extern void C_ARRATYP(
  BlockLocal *, int64_t &, int64_t &, bool FIRSTINDEX, Types, int64_t, int64_t);
extern void TYPF(
    ...
    if (symbol == Symbol::CHARCON) {
      ...
      if (
        ...
        if (NEXTC.TP != Types::INTS)
          ...
          switch (OPSYM) {
            case Symbol::PLUS: C.I = C.I + NEXTC.I; break;
            case Symbol::MINUS: C.I = C.I - NEXTC.I; break;
            case Symbol::TIMES: C.I = C.I * NEXTC.I; break;
            default: break;
          }
        }
      }
...
void OPTIONS(BlockLocal *bl) {
  ...
  switch (topology) {
    ...
    case Symbol::CLUSTERSY: highest_processor = TOPDIM - 1; break;
    case Symbol::HYPERCUBESY:
      ...
      break;
    case Symbol::LINESY:
    case Symbol::RINGSY: highest_processor = TOPDIM - 1; break;
    case Symbol::MESH2SY:
    case Symbol::TORUSSY: highest_processor = TOPDIM * TOPDIM - 1; break;
    case Symbol::MESH3SY:
...
void LOADVAL(Item &X) {
  if (
    standard_set.test(static_cast<int>(X.types)) || X.types == Types::PNTS ||
    X.types == Types::LOCKS)
    emit(34);
...
void SIMPLEEXPRESSION(BasicLocal *bx, SymbolSet &FSYS, Item &X) {
  ...
  while (symbol == Symbol::PLUS || symbol == Symbol::MINUS) {
    ...
    if (
      X.types == Types::PNTS && Y.types == Types::PNTS && OP == Symbol::MINUS) {
      X.types = Types::INTS;
      emit(53);
    } else {
  ...
  if (su.test(static_cast<int>(symbol))) {
    ...
    if (sv[static_cast<int>(X.types)]) {
      if (
        ...
        switch (OP) {
          case Symbol::EQL: emit(45); break;
          case Symbol::NEQ: emit(46); break;
          case Symbol::LSS: emit(47); break;
          case Symbol::LEQ: emit(48); break;
          case Symbol::GTR: emit(49); break;
          case Symbol::GEQ: emit(50); break;
          default: break;
        }
      } else
        error(35);
...
void COMPASSIGNEXP(BasicLocal *bx, SymbolSet &FSYS, Item &X) {
  ...
  switch (OP) {
    ...
    case Symbol::MINUSEQ:
      ...
      if (
        Z.types == Types::PNTS && Y.types == Types::PNTS &&
        OP == Symbol::MINUSEQ) {
        Z.types = Types::INTS;
        emit(53);
      } else {
        ...
        if (
          Z.types == Types::INTS || Z.types == Types::REALS ||
          Z.types == Types::CHARS) {
          if (OP == Symbol::PLUSEQ)
            emit(52);
          else
  ...
  if (COMPARATORS_SET.test(static_cast<int>(symbol)))
    ...
    if (type_compatible(X, Y))
      ...
      if (X.types == Types::REALS && Y.types == Types::INTS)
        ...
        standard_set[static_cast<int>(X.types)] || X.types == Types::PNTS ||
        X.types == Types::LOCKS)
        emit(38, SF);
      else {
        ...
        if (!X.is_address || X.types != Types::CHANS) {
          ...
          if (type_compatible(X, Y))
            ...
            if (X.types == Types::REALS && Y.types == Types::INTS)
              ...
              standard_set[static_cast<int>(X.types)] ||
              X.types == Types::PNTS) {
              emit(38, 0);
              emit(111);
            } else {
