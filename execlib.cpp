...
#define MAXDIM 10
  ...
  int HOPS(int SOURCE, int DEST) {
    ...
    switch (topology) {
      ...
      case Symbol::HYPERCUBESY: T1 = SOURCE;
        T2 = DEST;
        DIST = 0;
        while (T1 + T2 > 0) {
          if (T1 % 2 != T2 % 2)
        ...
        break;
      case Symbol::LINESY: DIST = abs(SOURCE - DEST);
        break;
      case Symbol::MESH2SY: T1 = TOPDIM;
        T2 = abs(SOURCE / T1 - DEST / T1);
        T3 = abs(SOURCE % T1 - DEST % T1);
        DIST = T2 + T3;
        break;
      case Symbol::MESH3SY: T2 = TOPDIM;
        T1 = TOPDIM * T2;
        T3 = SOURCE % T1;
        T4 = DEST % T1;
        ...
        break;
      case Symbol::RINGSY: T1 = abs(SOURCE - DEST);
        T2 = TOPDIM - T1;
        if (T1 < T2)
          DIST = T1;
        else
  ...
  int GRPDELAY(Interpreter* il, int lSOURCE, int DEST, int LEN) {
    ...
    switch (topology) {
      case Symbol::HYPERCUBESY: AVEFAN = TOPDIM;
        MAXFAN = TOPDIM;
        break;
      case Symbol::LINESY:
      case Symbol::RINGSY: AVEFAN = 1;
        MAXFAN = 2;
        break;
      case Symbol::MESH2SY:
      case Symbol::TORUSSY: AVEFAN = 2;
        MAXFAN = 3;
        break;
      case Symbol::MESH3SY: AVEFAN = 3;
        MAXFAN = 4;
        break;
      case Symbol::FULLCONNSY:
      case Symbol::CLUSTERSY: AVEFAN = static_cast<int>(round(
                                log(highest_processor + 1) / ln2
                              ));
        MAXFAN = AVEFAN;
        break;
      default: break;
  ...
  int GETRANK(ExlibLocal* el, int IROW, const int COOR[MAXDIM]) {
    ...
    int NDIM = il->MPICART[IROW][0];
    int RANK = 0;
    for (int I = 1; I <= NDIM; I++) {
      int THISMAX = il->MPICART[IROW][I];
      int THISCOOR = COOR[I];
      if (THISCOOR >= THISMAX) {
        if (il->MPIPER[IROW][I])
  ...
  void EXECLIB(Interpreter* il, ExLocal* el, PROCPNT CURPR, int lID) {
    ...
    switch (lID) {
      ...
      case 9: // ceil
        ...
        if (xl.R1 <= 0 || xl.R1 == trunc(xl.R1))
          il->S[CURPR->T] = static_cast<int>(trunc(xl.R1));
        else
          il->S[CURPR->T] = static_cast<int>(trunc(xl.R1)) + 1;
      ...
      case 10: // floor
        ...
        if (xl.R1 >= 0 || xl.R1 == trunc(xl.R1))
          il->S[CURPR->T] = static_cast<int>(trunc(xl.R1));
        else il->S[CURPR->T] = static_cast<int>(trunc(xl.R1)) - 1;
        il->RS[CURPR->T] = il->S[CURPR->T];
        il->S[CURPR->T] = RTAG;
        break;
      case 11: // sin
      ...
      case 38: // tolower
        ...
        break;
      case 39: xl.H1 = il->S[CURPR->T];
        if (xl.H1 > 96 && xl.H1 < 123)
          il->S[CURPR->T] = xl.H1 - 32;
        break;
      case 40: il->PS = PS::USERSTOP;
        break;
      case 41: CURPR->T = CURPR->T - 1;
        if (CURPR->FORKCOUNT > 1) {
          CURPR->STATE = State::BLOCKED;
          CURPR->PC -= 1;
          il->PROCTAB[CURPR->PROCESSOR].RUNPROC = nullptr;
          CURPR->FORKCOUNT -= 1;
        } else {
          il->PS = PS::FIN;
          CURPR->STATE = State::TERMINATED;
        }
        break;
      //      BEGIN
      //       H1 := S^[T-1];
    ...
    rnd = random();
        ...
        break;
      case 50: il->MPIINIT[CURPR->PROCESSOR] = true;
        CURPR->T = CURPR->T + 1;
      ...
      case 51: // finalize
        ...
        if (il->CHAN[il->CNUM].SEM == 0)
          ...
          il->S[CURPR->T] = -1;
        break;
      case 52: if (!il->MPIINIT[CURPR->PROCESSOR]) {
          il->PS = PS::MPIINITCHK;
          break;
        }
        ...
        il->S[CURPR->T] = MPISUCCESS;
        break;
      case 53: if (!il->MPIINIT[CURPR->PROCESSOR]) {
          il->PS = PS::MPIINITCHK;
          break;
        }
        if (CHKCOMM(&xl, il->S[CURPR->T - 1])) {
        ...
        xl.H1 = il->S[CURPR->T];
        il->S[xl.H1] = highest_processor + 1;
        if (il->NUMTRACE > 0)
          CHKVAR(il, xl.H1);
        CURPR->T = CURPR->T - 1;
        il->S[CURPR->T] = MPISUCCESS;
        break;
      case 54: if (!il->MPIINIT[CURPR->PROCESSOR]) {
          il->PS = PS::MPIINITCHK;
          break;
        }
        ...
        il->S[CURPR->T] = MPISUCCESS;
        break;
      case 55: if (!il->MPIINIT[CURPR->PROCESSOR]) {
          il->PS = PS::MPIINITCHK;
          break;
        }
      ...
      // with CHAN[CNUM]
        ...
        break;
      case 56: if (!il->MPIINIT[CURPR->PROCESSOR]) {
          il->PS = PS::MPIINITCHK;
          break;
        }
        if (il->S[CURPR->T] <= 1 ||
        ...
        il->S[CURPR->T] = MPISUCCESS;
        break;
      case 57: if (!il->MPIINIT[CURPR->PROCESSOR]) {
          il->PS = PS::MPIINITCHK;
          break;
        }
      ...
      case 58: {
        ...
        if (il->MPISEM == 0) {
          ...
          for (xl.I = 0; xl.I <= highest_processor; xl.I++) {
      ...
      case 59: {
        ...
        if (il->MPISEM == 0) {
          ...
          do {
            ...
            il->PTEMP->PDES->STATE = State::DELAYED;
            il->PTEMP = il->PTEMP->NEXT;
      ...
      case 61: {
        ...
        if (CHKCOMM(&xl, il->S[CURPR->T]) || il->S[CURPR->T - 1] < 0 ||
            ...
            !(il->S[CURPR->T - 3] == MPIINT || il->S[CURPR->T - 3] ==
              MPIFLOAT) ||
          ...
          if (il->MPITYPE == MPIINT) {
            for (xl.J = 0; xl.J < il->MPICNT; xl.J++) {
              ...
              for (xl.I = 0; xl.I <= highest_processor; xl.I++) {
                ...
                switch (il->MPIOP) {
                  case MPIMAX: if (xl.H2 < xl.H3)
                      xl.H2 = xl.H3;
                    break;
                  case MPIMIN: if (xl.H2 > xl.H3)
                      xl.H2 = xl.H3;
                    break;
                  case MPISUM: xl.H2 = xl.H2 + xl.H3;
                    break;
                  case MPIPROD: xl.H2 = xl.H2 * xl.H3;
                    break;
                  case MPILAND: if (xl.H3 != 0 && xl.H2 == 1)
                      xl.H2 = 1;
                    else
                      xl.H2 = 0;
                    break;
                  case MPILOR: if (xl.H3 != 0 || xl.H2 == 1)
                      xl.H2 = 1;
                    else
                      xl.H2 = 0;
                    break;
                  case MPILXOR: if (
                      (xl.H3 != 0 && xl.H2 == 0) || (xl.H3 == 0 && xl.H2 == 1))
                      xl.H2 = 1;
                    else
                      xl.H2 = 0;
                    break;
            ...
            for (xl.J = 0; xl.J < il->MPICNT; xl.J++) {
              ...
              for (xl.I = 0; xl.I <= highest_processor; xl.I++) {
                ...
                switch (il->MPIOP) {
                  case MPIMAX: if (xl.R2 < xl.R3)
                      xl.R2 = xl.R3;
                    break;
                  case MPIMIN: if (xl.R2 > xl.R3)
                      xl.R2 = xl.R3;
                    break;
                  case MPISUM: xl.R2 = xl.R2 + xl.R3;
                    break;
                  case MPIPROD: xl.R2 = xl.R2 * xl.R3;
                    break;
                }
              }
      ...
      case 62: {
        ...
        if (CHKCOMM(&xl, il->S[CURPR->T]) ||
            ...
            !(il->S[CURPR->T - 2] == MPIINT || il->S[CURPR->T - 2] ==
              MPIFLOAT) ||
            il->S[CURPR->T - 3] <= 0 || il->S[CURPR->T - 4] <= 0 ||
            il->S[CURPR->T - 5] <= 0 ||
            ((il->S[CURPR->T - 1] == MPILAND || il->S[CURPR->T - 1] == MPILOR ||
        ...
        if (il->MPISEM == 0) {
          ...
          for (xl.I = 0; xl.I <= highest_processor; ++xl.I) {
            for (xl.J = 0; xl.J < il->MPICNT; ++xl.J) {
              if (il->MPITYPE == MPIINT)
                il->S[il->MPIRES[xl.I] + xl.J] = il->S[
                  il->MPIRES[xl.H1] + xl.J];
              else {
                il->S[il->MPIRES[xl.I] + xl.J] = RTAG;
                il->RS[il->MPIRES[xl.I] + xl.J] = il->RS[
                  il->MPIRES[xl.H1] + xl.J];
              }
            }
          }
      ...
      default: print("execlib (%d) currently unimplemented\n", lID);
        break;
    }
  }
} // namespace Cstar
